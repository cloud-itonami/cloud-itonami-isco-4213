(ns pawnbroking.actor
  "PawnbrokingActor — the ISCO-08 4213 community pawnbroking actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one pawnbroking operation request
  ...
  The unconditional invariant: the PawnbrokingAdvisor can never
  directly commit a record the PawnbrokingGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [pawnbroking.advisor :as advisor]
            [pawnbroking.governor :as governor]
            [pawnbroking.store :as store]))

(defn build-graph
  "Build a compiled PawnbrokingActor graph. `store` implements
  `pawnbroking.store/Store`. `advisor` implements
  `pawnbroking.advisor/Advisor` (defaults to `mock-advisor`).
